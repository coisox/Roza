<?php
	...
	$prop = rozaGetUi(rozaGetParam('ROZA_UIID'));
	$data = [];
	...
	for($i=0; $i<count($prop); $i++) {
		...
		for($j=0; $j<count($prop[$i]['sourceparam']); $j++) $prop[$i]['sourceparam'][$j] = rozaReplaceParam($prop[$i]['sourceparam'][$j]);
		if(isset($prop[$i]['onclick'])) $prop[$i]['onclick'] = rozaReplaceParam($prop[$i]['onclick']);
		if(isset($prop[$i]['onchange'])) $prop[$i]['onchange'] = rozaReplaceParam($prop[$i]['onchange']);
		...
		if($prop[$i]['type']=='standardlist') {
			...
			for($j=0; $j<count($prop[$i]['list']); $j++) {
				if(isset($prop[$i]['onclick'])) $prop[$i]['list'][$j]['onclick'] = rozaReplaceField($prop[$i]['onclick'], $prop[$i]['list'][$j]);
				if(isset($prop[$i]['onchange'])) $prop[$i]['list'][$j]['onchange'] = rozaReplaceField($prop[$i]['onchange'], $prop[$i]['list'][$j]);
			}
		}
